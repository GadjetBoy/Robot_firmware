...
static uint8_t tx_sensor_data[] = {0,1,2,3,4,5,6,7};  // Holds data to send

volatile DRAM_ATTR uint8_t rx_buffer[SERVER_MAX_DATA_LEN];  // Stores received data
volatile DRAM_ATTR uint8_t int_val[NUMBER_OF_INTS];
 ...
 switch (event) {
    ...
    case ESP_GATTS_READ_EVT: {
      ESP_LOGI(BLE_TAG, "GATT_READ_EVT, conn_id %" PRIu16 ", trans_id %" PRIu32 ", handle %" PRIu16,
              ...
              read_rsp.attr_value.len = NUM_MOTORS;
              ...
              esp_ble_gatts_send_response(gatts_if,
                 ...
                 ESP_LOGI(BLE_TAG, "GATT_READ_EVT, INVALID REQUEST FROM :conn_id %" PRIu16 ", trans_id %" PRIu32 ", handle %" PRIu16,
                 param->read.conn_id, param->read.trans_id, param->read.handle);
      ...
      if (!param->write.is_prep){
         ...
         if (param->write.handle == gl_profile_tab[PROFILE_A_APP_ID].rx_char_handle) {
              ...
              if(len+recv_len<=SERVER_MAX_DATA_LEN){
                 memcpy((void*)rx_buffer+recv_len, param->write.value, len);
                 recv_len+=len;

                }
              else {
                 // overflow — probably a bug or garbled client write
                 ESP_LOGW(BLE_TAG, "Too much data: recv_len=%d + len=%d > %d",
              ...
              if(recv_len == SERVER_MAX_DATA_LEN){
                ...
                /* uint8_t start_meta = rx_buffer[0];
                 ...
                 // copydown integer values 
                   ...
                   memcpy((void*)int_val,(void*)rx_buffer + 1, NUMBER_OF_INTS);
                   
                 //copydown float values 
                   
                   memcpy((void*)float_val,(void*)rx_buffer+NUMBER_OF_INTS+1,NUMBER_OF_FLOATS*sizeof(float));
                   taskEXIT_CRITICAL(&ble_mux);
                // Log integers
                 for (int i = 0; i < NUMBER_OF_INTS; i++) {
                     ESP_LOGI(BLE_TAG, "Int[%d] = %u", i, int_val[i]);
                    }

                // Log floats
                 for (int i = 0; i < NUMBER_OF_FLOATS; i++) {
                     ESP_LOGI(BLE_TAG, "Float[%d] = %.2f", i, float_val[i]);
            ...
            ESP_LOGE(BLE_TAG, "invlaid write to the server\n");
        }
...
void BLE_app_main(void) {
  ESP_LOGI(BLE_TAG, "setting up BLE unit\n" );
     ...
     memset((void*)int_val, 0, sizeof(int_val)); 
