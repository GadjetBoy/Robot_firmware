...
void update_motor(Motor *m) {
    ...
    if(m->direction != m->prev_dir){
       ESP_LOGI(PID_TAG, "STOP MOTOR [%d]\n", m->m_num);  
       stop_motor(m->pwm_channel);
       vTaskDelay(pdMS_TO_TICKS(20));
       // Reset PID to prevent windup from previous direction
...
void pid_app_main(void){
    ...
    init_pid(&m1.pid,(10.00), (2.0), (0.0));  // Tune these values
    m1.pwm_channel = 0;
