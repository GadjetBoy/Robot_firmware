...
static int tx_sensor_data[NUM_MOTORS] = {0};  // Holds data to send
volatile DRAM_ATTR uint8_t rx_buffer[NUM_MOTORS] = {0};  // Stores received data
 ...
 switch (event) {
    ...
    case ESP_GATTS_ADD_CHAR_DESCR_EVT: {
      ...
      if (gl_profile_tab[PROFILE_A_APP_ID].tx_descr_uuid.uuid.uuid16 == param->add_char_descr.descr_uuid.uuid.uuid16) {
         ...
         /* else{
           ESP_LOGI(GATTS_TAG, "ADD_RX_DESCR_EVT, status %d, attr_handle %d, service_handle %d",
                  param->add_char_descr.status, param->add_char_descr.attr_handle,
                  param->add_char_descr.service_handle);

            gl_profile_tab[PROFILE_A_APP_ID].rx_descr_handle = param->add_char_descr.attr_handle;
            }*/
    ...
    case ESP_GATTS_WRITE_EVT: {
         ...
         if (param->write.handle == gl_profile_tab[PROFILE_A_APP_ID].rx_char_handle) {
                ...
                size_t len = (param->write.len < NUM_MOTORS) ? param->write.len : NUM_MOTORS;

                taskENTER_CRITICAL(&mux1);
                memcpy((uint8_t *)rx_buffer, param->write.value, len);
                taskEXIT_CRITICAL(&mux1);

                ESP_LOGI(GATTS_TAG, "Received data (len %"PRIu16"):", len);
                ESP_LOG_BUFFER_HEX(GATTS_TAG,(const uint8_t *) rx_buffer, len);

                
                memcpy(rsp.attr_value.value,(const uint8_t *) rx_buffer, len);
...
void ble_send_sensor_data_task(void *pvParameters) {
  while(1) {
    for(int i=0;i<NUM_MOTORS;i++){
     ...
     tx_sensor_data[i]= Data_Buff[i]/(28*GEAR_RATIO);
    ...
    if (gl_profile_tab[PROFILE_A_APP_ID].tx_notifications_enabled && is_connected) {
      ...
      esp_ble_gatts_send_indicate(
       ...
       (uint8_t *) tx_sensor_data,
        false // false = Notification (no ack), true = Indication (ack)
      );
    }
    vTaskDelay(pdMS_TO_TICKS(10)); // Adjust interval as needed
  }
}
