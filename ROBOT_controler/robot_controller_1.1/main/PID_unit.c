...
float pos_target_val[NUMBER_OF_MOTORS]; 
float neg_target_val[NUMBER_OF_MOTORS];
volatile uint8_t control;
float map_val;
float motor_pos[NUMBER_OF_MOTORS];
...
float compute_pid(PIDController *pid, float error,uint8_t i){
 ...
 //integral
 (pid->integral) += (error * dt); // Use float
 ...
 U = (U > MAX_CONTROL_SIGNAL) ? MAX_CONTROL_SIGNAL : 
      (U < MIN_CONTROL_SIGNAL) ? MIN_CONTROL_SIGNAL : U;

 if(isnan(error) || isnan(pid->integral)) {
        pid->integral = 0;
        pid->prev_error = 0;
        return 0;
    }
 ...
 ESP_LOGI(PWM_TAG, "control_signal[%d]= %.2f",i,U);
 return U ;
}
...
void update_motor(Motor *m) {
    ...
    ESP_LOGI(PWM_TAG, "pcnt_count_begin[%d]= %.2f",m->m_num,((float)pcnt_count[m->m_num-1]));
    ESP_LOGI(PWM_TAG, "pcnt_count_at_task_upadte[%d]= %.2f",m->m_num,(m->current_position)*EN_PPR);
    ...
    if(error<0){
      m->direction = -1;
     ESP_LOGI(PWM_TAG, " 0x%x drive motors", LEDC_MODEB);  
    }
    else{
        m->direction = 1;
    ESP_LOGI(PWM_TAG, " 0x%x drive motors", LEDC_MODEA);  
    }

    float pwm = fabs(output)*map_val;
    m->pwm = pwm;
    ESP_LOGI(PWM_TAG, "PWM_signal[%d]= %.2f",m->m_num,pwm);

    ESP_LOGI(PWM_TAG, "target_POS[%d]= %.2f",m->m_num,m->target_position);

    ESP_LOGI(PWM_TAG, "pcnt_count_end[%d]= %.2f",m->m_num,((float)pcnt_count[m->m_num-1]));
...
void handle_state_machine() {
    switch(current_state) {
        ...
        case STATE_MOT2_NEG:  
            ...
            break;
        case STATE_IDLE:
                  ESP_LOGI(PWM_TAG, "state IDLE state machine: 0x%x", current_state); 

            break;
        
        
        default:
              ESP_LOGE(PWM_TAG, "Invalid state for handle stae machine: 0x%x", current_state);
            break;
    }
}
...
void mode_set(void){
    ...
    if(control!=last_control){
     ...
     switch(control){
          ...
          break;
          default:
             ESP_LOGI(PWM_TAG, "no valid mode avilable");
             current_state = STATE_IDLE;
          break;
       }
        last_control=control;
   }
  ESP_LOGI(PWM_TAG, "control= %d",control);
}
void update_pos(void){
    int32_t temp_motor_pos_buf[NUMBER_OF_MOTORS]; 
     taskENTER_CRITICAL(&pcnt_mux);
     memcpy(temp_motor_pos_buf,(void *)pcnt_count,sizeof(int32_t)*NUMBER_OF_MOTORS);
       ...
       for(int i =0;i<NUMBER_OF_MOTORS;i++){
        ...
        switch(current_state) {
            ...
            case STATE_IDLE:
                ...
                break;
            default:
              ESP_LOGE(PWM_TAG, "Invalid state: 0x%x", current_state);
...
void pid_app_main(void){
    ...
    init_pid(&m1.pid, 2.50, 0.5, 1.50);  // Tune these values
    m1.pwm_channel = 0;
    
    // Motor 2 Initialization
    m2.m_num = 2;
    m2.target_position = 0;
    init_pid(&m2.pid, 2.50, 0.5, 1.50);  // Tune these values
    m2.pwm_channel = 1;
    ...
    init_pid(&m3.pid, 2.50, 0.5, 1.50);  // Tune these values
    m3.pwm_channel = 2;
    
    // Motor 4 Initialization
    m4.m_num = 4;
    m4.target_position = 0;
    init_pid(&m4.pid, 2.50, 0.5, 1.50);  // Tune these values
    m4.pwm_channel = 3;
