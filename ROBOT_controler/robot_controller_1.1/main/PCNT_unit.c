...
#define TAG  "PCNT UNIT"

volatile DRAM_ATTR int32_t pcnt_count[NUM_ENCORDERS] __attribute__((aligned(4))) = {0};
...
void PCNT_Read_Task( void *pvParameters){
 ...
 ESP_LOGI(TAG, "Read Task Stack: %u", uxTaskGetStackHighWaterMark(NULL));

 int32_t temp_counts[NUM_ENCORDERS];
...
void PCNT_Print_Task(void *pvParameters){
 ...
 int32_t received_counts[NUM_ENCORDERS];
 for(;;){
