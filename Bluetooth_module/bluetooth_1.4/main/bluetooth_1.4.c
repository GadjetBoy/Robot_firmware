      ...
      if (!param->write.is_prep){
         ...
         if (param->write.handle == gl_profile_tab[PROFILE_A_APP_ID].rx_char_handle) {
              ...
              ESP_LOG_BUFFER_HEX(GATTS_TAG, rx_buffer, len);
              	...
              	ESP_LOGI(GATTS_TAG, "invalid data lenght (len %"PRIu16"):", len);
              }
            }
...
void exec_write_event_env(prepare_type_env_t *prepare_write_env, esp_ble_gatts_cb_param_t *param){
    if (param->exec_write.exec_write_flag == ESP_GATT_PREP_WRITE_EXEC){
        ...
        memcpy(rx_float_buff,(float*)prepare_write_env->prepare_buf,prepare_write_env->prepare_len/sizeof(float));
        for(int i=0;i<NUMBER_OF_MOTORS;i++){
          ESP_LOGI(GATTS_TAG, "recieved floats %f",rx_float_buff[i]);
        }
    }
    else{
