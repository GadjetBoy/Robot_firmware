...
#define CPG_UPDATE_RATE_MS 2 // 50 Hz update rate
    ...
    while (1) {
      ...
      if (xSemaphoreTake(cpg_params_mutex, pdMS_TO_TICKS(10)) == pdTRUE) {
        ...
        for (int i = 0; i < NUM_OSCILLATORS; i++) { 
            ...
            cpg_network[i].amplitude += 0.01f * (target_amp - cpg_network[i].amplitude)*CPG_DT; // smooth correction

            
            // --- Bias correction (prevent drift) ---
            float target_offset = (i % 2 != 0) ? CPG_network_pram.knee_offset:CPG_network_pram.hip_offset;
            cpg_network[i].offset += 0.05f * (target_offset - cpg_network[i].offset)*CPG_DT;
