...
void IRAM_ATTR Data_Recive_task(void *arg) {
    ...
    encoder_packet_t msg;

    while (1) {
        ulTaskNotifyTake(pdTRUE, portMAX_DELAY);
       
        if (xQueueReceive(context->recieve_queue, &msg,0) == pdTRUE) {

            xQueueOverwrite(encoder_data_queue, &msg);
...
void app_main(void) {
    ...
    encoder_data_queue = xQueueCreate(1, sizeof(encoder_packet_t)); 
