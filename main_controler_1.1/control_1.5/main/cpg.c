...
void IRAM_ATTR cpg_timer_callback(void* arg) {
    ...
    xQueueSendFromISR(cpg_update_queue, &Cmd, &xHigherPriorityTaskWoken);
...
void cpg_update_task(void *arg) {
    ...
    while (1) {
      ...
      if (xQueueReceive(cpg_update_queue, &update_cmd, portMAX_DELAY)) {
         // Check for missed updates (optional - for monitoring)
          if (update_cmd.sequence != last_sequence + 1) {
                ESP_LOGW(TAG, "CPG update missed: expected %lu, got %lu", 
                        last_sequence + 1, update_cmd.sequence);
            }
            last_sequence = update_cmd.sequence;
        }
      ...
      if (xSemaphoreTake(cpg_params_mutex, pdMS_TO_TICKS(10)) == pdTRUE) {
        ...
        // Only send the command array if we successfully updated it.
          if(xQueueSend(motor_command_queue, cmd, pdMS_TO_TICKS(100)) !=pdTRUE){
             ESP_LOGE(TAG, "CPG command send failed :que full!");
            }
...
void mot_command_app_main(void) {
    ...
    cpg_update_queue = xQueueCreate(100, sizeof(cpg_update_cmd_t));
