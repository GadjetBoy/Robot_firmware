...
#define MOTOR1_PINA 1
#define MOTOR1_PINB 2
...
void copy_ble_recieve_data_task(void *pvParameters) {
    ...
    while (1) {
        if (xQueueReceive(BLE_recieve_queue, &local_rx_buffer, portMAX_DELAY) == pdTRUE) {
            ...
            // Log the raw data as hex for generic handling
            // (Application can parse based on data structure here)
           for (int i = 0; i < local_rx_buffer.length; i++){
             ESP_LOGI("BLE", "RX[%d] = 0x%02X (%c)",
             i,
             local_rx_buffer.data[i],
             local_rx_buffer.data[i]);
            }
        }
...
void runmotors_Task(void *pvParameters){
while (1) {

     gpio_set_level(MOTOR1_PINA,1);
     gpio_set_level(MOTOR1_PINB,0);
...
void app_main(void) {
    ...
    if (BLE_recieve_queue == NULL) {
     ...
     gpio_reset_pin(MOTOR1_PINA); 
     gpio_reset_pin(MOTOR1_PINB);

     gpio_set_direction(MOTOR1_PINA, GPIO_MODE_OUTPUT);
     gpio_set_direction(MOTOR1_PINB, GPIO_MODE_OUTPUT);
    ...
    xTaskCreatePinnedToCore(runmotors_Task,
                           "recieve_data_task",
                           2048,
                           NULL,
                           8,
