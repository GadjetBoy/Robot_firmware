...
void ads1292_Init(void)
{
    ...
    ads1292_Reg_Write(spi, ADS1292_REG_CH1SET, 0b00000001); //Ch 1 enabled, gain 6, connected to electrode in 
    vTaskDelay(10 / portTICK_PERIOD_MS);

    ads1292_Reg_Write(spi, ADS1292_REG_CH2SET, 0b00000001); //Ch 2 enabled, gain 6, connected to electrode in  
    ...
    uint8_t id = ads1292_Reg_Read(ADS1292_REG_ID,1);            // ID register
    printf("ADS1292R ID = 0x%02X\n", id);

    ads1292_Reg_Write(spi, ADS1292_REG_CONFIG1, 0b00000000);      //Set sampling rate to 125 SPS
    vTaskDelay(10 / portTICK_PERIOD_MS);

    ads1292_Reg_Write(spi, ADS1292_REG_CONFIG2, 0b10100011);    //Lead-off comp off, test signal enabled
    vTaskDelay(10 / portTICK_PERIOD_MS);

    ads1292_Reg_Write(spi, ADS1292_REG_CH1SET, 0b00000101); //Ch 1 enabled, gain 6, connected to electrode in 
    vTaskDelay(10 / portTICK_PERIOD_MS);

    ads1292_Reg_Write(spi, ADS1292_REG_CH2SET, 0b00000101); //Ch 2 enabled, gain 6, connected to electrode in  
    vTaskDelay(10 / portTICK_PERIOD_MS);
...
void ads1292_Reset(void) {
    ...
    vTaskDelay(100 / portTICK_PERIOD_MS);
    gpio_set_level(ADS1292_PWDN_PIN, HIGH);
    vTaskDelay(20 / portTICK_PERIOD_MS);  // Wait 18 tCLK cycles (~35µs)
}
...
uint8_t ads1292_Reg_Read(unsigned char START_ADDRESS, uint8_t num_registers) {
    uint8_t tx_buffer[3] = {0};
    uint8_t rx_buffer[3] = {0};
    
    // Format: [RREG + address, number of registers - 1, dummy byte]
    tx_buffer[0] = START_ADDRESS | RREG;  // Read command + address
    tx_buffer[1] = num_registers - 1;     // Number of registers to read - 1
    tx_buffer[2] = 0x00;
    
    spi_transaction_t t = {
        .length = 24,  // 3 bytes * 8 bits = 24 bits
        .tx_buffer = tx_buffer,
        .rx_buffer = rx_buffer,
        .flags = 0  // Full duplex
    };
    ...
    // The register value is in the last byte received
    // After sending [command, N-1], we get [dummy, dummy, register_value]
    return rx_buffer[2];
}
