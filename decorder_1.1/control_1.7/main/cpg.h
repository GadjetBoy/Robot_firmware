...
#include "control_1.7.h"  // Assuming this is the main control header
...
#define TOL                     100     // Position tolerance (encoder ticks)
#define STOP_THRESHOLD          30       // "Stopped" threshold (ticks)
...
// CPG Globals
extern Oscillator cpg_network[NUM_OSCILLATORS];  // Oscillator array
extern osc_pram CPG_network_pram;                // Tunable params
extern float coupling_weights[NUM_OSCILLATORS][NUM_OSCILLATORS];  // Coupling matrix
extern float phase_offsets[NUM_OSCILLATORS][NUM_OSCILLATORS];    // Phase offset matrix
extern volatile SequenceMode current_mode;  // Active gait mode
...
// Gait Setters (easy to call)
void set_gait_idle(void);
void set_gait_crawl(void);
void set_gait_trot(void);  // Alias for TURTLE
